...
internal class TestSimpleExRecordData : HavokData<TestSimpleExRecord>
{
    public TestSimpleExRecordData(HavokType type, TestSimpleExRecord instance) : base(type, instance) { }
    ...
    public override bool TryGetField<TGet>(string fieldName, [MaybeNull] out TGet value)
    {
        ...
        switch (fieldName)
        {
            ...
            case "nested":
                {
                    if (instance.m_nested is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            case "m_record":
    ...
    public override bool TrySetField<TSet>(string fieldName, TSet value)
    {
        switch (fieldName)
        {
            ...
            case "nested":
                {
                    if (value is not TestSimpleExRecord.Nested castValue) return false;
                    instance.m_nested = castValue;
                    return true;
                }
            case "m_record":
            case "record":
