...
internal class hkbLayerGeneratorData : HavokData<hkbLayerGenerator>
{
    public hkbLayerGeneratorData(HavokType type, hkbLayerGenerator instance) : base(type, instance) { }
    ...
    public override bool TryGetField<TGet>(string fieldName, [MaybeNull] out TGet value)
    {
        ...
        switch (fieldName)
        {
            ...
            case "layers":
                {
                    if (instance.m_layers is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
    ...
    public override bool TrySetField<TSet>(string fieldName, TSet value)
    {
        switch (fieldName)
        {
            ...
            case "layers":
                {
                    if (value is not List<hkbLayer?> castValue) return false;
                    instance.m_layers = castValue;
                    return true;
                }
            case "m_indexOfSyncMasterChild":
            case "indexOfSyncMasterChild":
            ...
            case "flags":
                {
                    if (value is hkbLayerGenerator.LayerFlagBits castValue)
                    {
                    ...
                    {
                        instance.m_flags = (hkbLayerGenerator.LayerFlagBits)ushortValue;
                        return true;
                    }
                    return false;
                }
