...
internal class hkaiWorldCommandsDoSynchronousProcessingRunData : HavokData<DoSynchronousProcessingRun>
{
    public hkaiWorldCommandsDoSynchronousProcessingRunData(HavokType type, DoSynchronousProcessingRun instance) : base(type, instance) { }
    ...
    public override bool TryGetField<TGet>(string fieldName, [MaybeNull] out TGet value)
    {
        ...
        switch (fieldName)
        {
            ...
            case "m_processingSettings":
            case "processingSettings":
                {
                    if (instance.m_processingSettings is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            default:
    ...
    public override bool TrySetField<TSet>(string fieldName, TSet value)
    {
        switch (fieldName)
        {
            ...
            case "processingSettings":
                {
                    if (value is not hkaiWorldProcessingSettings castValue) return false;
                    instance.m_processingSettings = castValue;
                    return true;
                }
            default:
                return false;
