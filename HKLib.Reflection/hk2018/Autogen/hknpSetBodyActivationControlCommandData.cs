...
using Enum = HKLib.hk2018.hknpActivationControl.Enum;

namespace HKLib.Reflection.hk2018;

internal class hknpSetBodyActivationControlCommandData : HavokData<hknpSetBodyActivationControlCommand>
{
    public hknpSetBodyActivationControlCommandData(HavokType type, hknpSetBodyActivationControlCommand instance) : base(type, instance) { }
    ...
    public override bool TryGetField<TGet>(string fieldName, [MaybeNull] out TGet value)
    {
        ...
        switch (fieldName)
        {
            ...
            case "m_activationControl":
            case "activationControl":
                {
                    if (instance.m_activationControl is TGet castValue)
                    {
                        value = castValue;
                        return true;
                    }
                    if ((byte)instance.m_activationControl is TGet byteValue)
                    {
                        value = byteValue;
                        return true;
                    }
            ...
            case "activationControl":
                {
                    ...
                    {
                        instance.m_activationControl = castValue;
                        return true;
                    }
                    if (value is byte byteValue)
                    {
                        instance.m_activationControl = (Enum)byteValue;
                        return true;
                    }
                    return false;
                }
