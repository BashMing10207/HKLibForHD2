...
internal class hclBendStiffnessConstraintSetMxSingleData : HavokData<hclBendStiffnessConstraintSetMx.Single>
{
    public hclBendStiffnessConstraintSetMxSingleData(HavokType type, hclBendStiffnessConstraintSetMx.Single instance) : base(type, instance) { }
    ...
    public override bool TryGetField<TGet>(string fieldName, [MaybeNull] out TGet value)
    {
        ...
        switch (fieldName)
        {
            ...
            case "invMassA":
                {
                    if (instance.m_invMassA is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            case "m_invMassB":
            case "invMassB":
                {
                    if (instance.m_invMassB is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            case "m_invMassC":
            case "invMassC":
                {
                    if (instance.m_invMassC is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            case "m_invMassD":
            case "invMassD":
                {
                    if (instance.m_invMassD is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            case "m_particleA":
