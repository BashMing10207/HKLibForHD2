...
internal class hkbAttributeModifierData : HavokData<hkbAttributeModifier>
{
    public hkbAttributeModifierData(HavokType type, hkbAttributeModifier instance) : base(type, instance) { }
    ...
    public override bool TryGetField<TGet>(string fieldName, [MaybeNull] out TGet value)
    {
        ...
        switch (fieldName)
        {
            ...
            case "assignments":
                {
                    if (instance.m_assignments is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            default:
    ...
    public override bool TrySetField<TSet>(string fieldName, TSet value)
    {
        switch (fieldName)
        {
            ...
            case "assignments":
                {
                    if (value is not List<hkbAttributeModifier.Assignment> castValue) return false;
                    instance.m_assignments = castValue;
                    return true;
                }
            default:
                return false;
