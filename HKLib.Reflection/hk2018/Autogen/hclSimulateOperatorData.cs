...
internal class hclSimulateOperatorData : HavokData<hclSimulateOperator>
{
    public hclSimulateOperatorData(HavokType type, hclSimulateOperator instance) : base(type, instance) { }
    ...
    public override bool TryGetField<TGet>(string fieldName, [MaybeNull] out TGet value)
    {
        ...
        switch (fieldName)
        {
            ...
            case "m_simulateOpConfigs":
            case "simulateOpConfigs":
                {
                    if (instance.m_simulateOpConfigs is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            default:
    ...
    public override bool TrySetField<TSet>(string fieldName, TSet value)
    {
        switch (fieldName)
        {
            ...
            case "simulateOpConfigs":
                {
                    if (value is not List<hclSimulateOperator.Config> castValue) return false;
                    instance.m_simulateOpConfigs = castValue;
                    return true;
                }
            default:
                return false;
