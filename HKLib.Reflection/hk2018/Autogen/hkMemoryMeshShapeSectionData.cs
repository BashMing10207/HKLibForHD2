...
internal class hkMemoryMeshShapeSectionData : HavokData<hkMemoryMeshShape.Section>
{
    public hkMemoryMeshShapeSectionData(HavokType type, hkMemoryMeshShape.Section instance) : base(type, instance) { }
    ...
    public override bool TryGetField<TGet>(string fieldName, [MaybeNull] out TGet value)
    {
        ...
        switch (fieldName)
        {
            ...
            case "m_indexBufferOffset":
            case "indexBufferOffset":
                {
                    if (instance.m_indexBufferOffset is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            default:
    ...
    public override bool TrySetField<TSet>(string fieldName, TSet value)
    {
        switch (fieldName)
        {
            ...
            case "indexBufferOffset":
                {
                    if (value is not int castValue) return false;
                    instance.m_indexBufferOffset = castValue;
                    return true;
                }
            default:
                return false;
