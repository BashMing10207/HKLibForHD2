...
internal class hknpMotionAllocatedTraceData : HavokData<hknpMotionAllocatedTrace>
{
    public hknpMotionAllocatedTraceData(HavokType type, hknpMotionAllocatedTrace instance) : base(type, instance) { }
    ...
    public override bool TryGetField<TGet>(string fieldName, [MaybeNull] out TGet value)
    {
        ...
        switch (fieldName)
        {
            ...
            case "motionInfo":
                {
                    if (instance.m_motionInfo is not TGet castValue) return false;
                    value = castValue;
                    return true;
                }
            default:
    ...
    public override bool TrySetField<TSet>(string fieldName, TSet value)
    {
        switch (fieldName)
        {
            ...
            case "motionInfo":
                {
                    if (value is not hknpMotionCinfo castValue) return false;
                    instance.m_motionInfo = castValue;
                    return true;
                }
            default:
                return false;
